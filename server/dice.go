...
func (n Node) render(ind string, rr int, rcok bool) (string, string, string) {
	return n.sp.render(n, ind, rr, rcok)
}
func (GroupExpr) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	r1, r2, r3 := n.child[0].render(ind, rr, rcok)
...
func (sp Natural) render(_ Node, _ string, rr int, _ bool) (string, string, string) {
	return fmt.Sprintf("%d", sp.n), resultInteger(sp.n, rr), ""
}
func (sp Sum) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	return renderSumProd(n, ind, sp.ops, rr, rcok)
}
func (sp Prod) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	return renderSumProd(n, ind, sp.ops, rr, rcok)
}
func renderSumProd(n Node, ind string, ops []string, rr int, rcok bool) (string, string, string) {
	if len(n.child) == 1 {
		return n.child[0].sp.render(n.child[0], ind, rr, rcok)
	}
	r1, r2, r3 := "", resultInteger(n.value(), rr), ""
	rComment, c_rcok := renderRollComment(n, rr, rcok)
	r2 += rComment
	for i, c := range n.child {
		r1a, _, r3a := c.render(ind, RR_NONE, c_rcok)
		effectiveOp := ops[i]
		if effectiveOp == "*" {
			effectiveOp = "×"
		}
...
func (sp Dice) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	...
	result := resultInteger(n.value(), rr)
	if rr == RR_NONE {
...
func (sp Stats) render(n Node, ind string, _ int, _ bool) (string, string, string) {
	...
	details := ""
	for _, c := range n.child {
		_, _, detail := c.render(ind, RR_NONE, false)
		details += detail
	}
	return fmt.Sprintf("%s\n%s", intro, scoreText), "", details
}
func (sp DeathSave) render(n Node, ind string, rr int, _ bool) (string, string, string) {
	event := ""
	value := n.value()
	switch {
	case value == 1:
	...
	_, _, details := n.child[0].render(ind, RR_NONE, false)
	return fmt.Sprintf("a death saving throw, and %s", event), "", details
}
func (sp Labeled) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	if sp.label == "" {
		return n.child[0].render(ind, rr, rcok)
	}
	rComment, c_rcok := renderRollComment(n, rr, rcok)
	switch rr {
	case RR_TOP:
		r1, r2, r3 := n.child[0].render(ind, rr, c_rcok)
		return r1, fmt.Sprintf("%s %s%s", r2, sp.label, rComment), r3
	case RR_NONE:
		r1none, _, _ := n.child[0].render("  "+ind, RR_NONE, false)
		r1, r2, r3 := n.child[0].render("  "+ind, RR_DETAIL, false)
		return r1none, r2, fmt.Sprintf("\n%s*%s =* %s *%s*%s%s", ind, r1, r2, sp.label, rComment, r3)
	case RR_DETAIL:
		r1, r2, r3 := n.child[0].render("  "+ind, rr, false)
		return r1, fmt.Sprintf("%s *%s*%s", r2, sp.label, rComment), r3
	default:
		panic("invalid render request in Labeled.render")
	}
}
func (sp CommaList) render(n Node, ind string, rr int, rcok bool) (string, string, string) {
	r1, r2, r3 := "", "", ""
	for i, c := range n.child {
		r1a, r2a, r3a := c.render(ind, rr, rcok)
		if i > 0 {
			r1 += ", "
			r2 += ", "
		}
